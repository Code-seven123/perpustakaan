...
<div class="w-full flex justify-center gap-10">
    <div class="bg-white rounded-lg shadow-lg p-8 w-96">
        ...
        <button class="w-full mt-4 bg-green-500 text-white py-2 rounded-md hover:bg-green-700 transition duration-200 transform hover:scale-105" onclick="window.location.href = './'">Kembali</button>
    </div>
    <div class="bg-white rounded-lg shadow-lg p-8 w-96">
        ...
        <form method="post">
            ...
            <div class="mb-4">
                <label for="tgltake" class="block text-sm font-medium text-gray-700">User List</label>
                <select name="userid" class="input-field mt-1 block w-full border border-gray-300 rounded-md p-2 focus:outline-none">
                    <option value="" disabled>-- Pilih User --</option>
                    <?php
                        $userstmt = $conn->query("SELECT UserID, Username, NamaLengkap FROM user");
                        $user = $userstmt->fetchAll();
                        foreach($user as $value) {
                    ?>
                    <option value="<?= $value['UserID'] ?>"><?= $value['Username'] ?> | <?= $value['NamaLengkap'] ?></option>
                    <?php } ?>
                </select>
            </div>
